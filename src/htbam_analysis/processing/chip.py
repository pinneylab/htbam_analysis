...
class ImageSettings:
    ...
    circle_parameter1_index = 50
    circle_parameter2_index = 40

    def __init__(self):
        pass


def _quadrilateral_interpolation(corners, device_dims):
    """
    Grid the chip using the corners as vertices and the dims as the number of lattice points
    in the x and y directions.

    Arguments:
        (tuple | namedtuple) corners: corner positions of the form
            ((ULx, ULy),(URx, URy),(LLx, LLy),(LRx, LRy))
        (object) device_dims: chip dimensions object with .x and .y attributes

    Returns:
        (np.ndarray) a 2-D array of lattice (x, y) coordinates
    """
    ...
    left = interp(corners.ul, corners.bl, device_dims.y)
    right = interp(corners.ur, corners.br, device_dims.y)
    mesh = [interp(p1, p2, device_dims.x) for p1, p2 in zip(left, right)]
    return np.stack(mesh, axis=1)


def _make_stamps(image_path, centers, device_dims, pinlist, stamp_width, pinlist_feature_title="MutantID"):
    """
    Creates Stamp objects from an image using calculated center positions.

    Arguments:
        (str) image_path: path to the image file
        (np.ndarray) centers: array of chamber center positions
        (object) device_dims: chip dimensions object with .x and .y attributes
        (pd.DataFrame) pinlist: pinlist dataframe
        (float) stamp_width: width of the stamp
        (str) pinlist_feature_title: column name in pinlist for feature IDs

    Returns:
        (np.ndarray) a 2-D array of Stamp objects
    """
    ...
    slices = np.apply_along_axis(makeSlice, 2, centers, *[(stamp_width)])
    ...
    for x, y in indices.reshape((xdim * ydim, 2)):
        ...
        a[x, y] = Stamp(
            ...
            s,
            (x + 1, y + 1),
            pinlist.loc[x + 1, y + 1][pinlist_feature_title],
        )
    return a


def _find_chamber_in_stamp(stamp, coerce_center=False, chamber_radius=None, outer_chamber_bound=None, circle_para1_index=None, circle_para2_index=None):
    """
    Uses Hough transform to find a chamber in a stamp.

    Arguments:
        (Stamp) stamp: the stamp object to find chamber in
        (bool) coerce_center: whether to coerce center to image center
        (int) chamber_radius: radius of chamber to search for
        (int) outer_chamber_bound: outer boundary for search
        (int) circle_para1_index: Hough gradient parameter 1
        (int) circle_para2_index: Hough gradient parameter 2

    Returns:
        None (modifies stamp in place)
    """
    if chamber_radius is None:
        chamber_radius = Stamp.chamberrad
    if outer_chamber_bound is None:
        outer_chamber_bound = Stamp.outerchamberbound
    if circle_para1_index is None:
        circle_para1_index = Stamp.circlePara1Index
    if circle_para2_index is None:
        circle_para2_index = Stamp.circlePara2Index

    img = stamp.data

    if coerce_center:
        p = Stamp.circularSubsection(
            img, (int(img.shape[0] / 2), int(img.shape[0] / 2)), chamber_radius
        )
        stamp.chamber = Chamber(img, p["mask"], p["center"], p["radius"])
        return
    ...
    minRad = chamber_radius
    maxRad = minRad + outer_chamber_bound
    ...
    circles = cv2.HoughCircles(
        ...
        2,
        10,
        param1=circle_para1_index,
        param2=circle_para2_index,
        minRadius=minRad,
        maxRadius=maxRad,
    )

    search_para1 = circle_para1_index
    while type(circles) is not np.ndarray and search_para1 > 5:
        circles = cv2.HoughCircles(
            cimg,
            cv2.HOUGH_GRADIENT,
            2,
    ...
    if not np.any(circles):
        m = "No chamber border found for chamber {}".format(str(stamp.index))
        warnings.warn(m)
        stamp.chamber = Chamber.BlankChamber()
        return
...
def _find_button_in_stamp(stamp, search_spacing=7, radius=15, tile_width=110, tile_height=110, 
                           refining_range=7, min_radius=9, std_cutoff=0.9, bounding_inset_ratio=0.3):
    """
    Button finding algorithm using grid search optimization.
    Searches sparse grid of tile position centers, finds optimum, then refines by searching local
    neighborhood. Then fits the radius and re-fits the center position after each decrease in radius.

    Arguments:
        (Stamp) stamp: the stamp object to find button in
        (int) search_spacing: spacing for initial grid search
        (int) radius: initial button radius
        (int) tile_width: tile width
        (int) tile_height: tile height
        (int) refining_range: refinement search range
        (int) min_radius: minimum radius
        (float) std_cutoff: standard deviation cutoff for termination
        (float) bounding_inset_ratio: inset ratio for bounding box

    Returns:
        None (modifies stamp in place)
    """
    imagestamp = stamp.data
    maxI = 0
    bestSpotParams = None
    fitRadius = radius
    localBGRadius = radius * 2
    boundingInset = int(tile_width * bounding_inset_ratio)

    # Crude initial fit of center position (sparse initial search grid, entire image stamp) by maximizing summed intensity
    for xIndex in range(boundingInset, tile_width - boundingInset, search_spacing):
        for yIndex in range(boundingInset, tile_height - boundingInset, search_spacing):
            features = Stamp.circularSubsection(imagestamp, (xIndex, yIndex), fitRadius)
            summedI = np.nansum(features["intensities"])
            if summedI > maxI:
                maxI = deepcopy(summedI)
                bestSpotParams = deepcopy(features)

    # If the image is perfectly black in the bounding region, use center as placeholder
    if not bestSpotParams:
        warnmsg = "No intensity observed for chamber {}".format(stamp.index)
        warnings.warn(warnmsg)
        bestSpotParams = deepcopy(
            Stamp.circularSubsection(
                imagestamp, (int(tile_width / 2), int(tile_height / 2)), fitRadius
            )
        )
    ...
    for xIncrement in np.linspace(
        bestSpotParams["center"][0] - refining_range,
        bestSpotParams["center"][0] + refining_range,
        num=2 * refining_range,
        dtype=int,
    ):
        for yIncrement in np.linspace(
            bestSpotParams["center"][1] - refining_range,
            bestSpotParams["center"][1] + refining_range,
            num=2 * refining_range,
            dtype=int,
        ):
    ...
    while bestSpotParams["radius"] > min_radius:
        fitRadius -= 1
        maxIAtRadius = 0
        bestParamsAtRadius = bestSpotParams

        for xIncrement in np.linspace(
            bestParamsAtRadius["center"][0] - refining_range,
            bestParamsAtRadius["center"][0] + refining_range,
            num=2 * refining_range,
            dtype=int,
        ):
            for yIncrement in np.linspace(
                bestParamsAtRadius["center"][1] - refining_range,
                bestParamsAtRadius["center"][1] + refining_range,
                num=2 * refining_range,
                dtype=int,
            ):
        ...
        if np.nanstd(bestParamsAtRadius["intensities"]) < std_cutoff * refStdDev:
...
def process_image(
    ...
    corners: tuple,
    device_dims: tuple,
    find_buttons: bool = False, 
    find_chambers: bool = False,
    stamp_width: float = 100,
    reference_chip = None,
    save_summary_images: bool = False,
    coerce_chamber_center: bool = False,
    coerce_button_center: bool = False,
        ):
    """
    Process an image to extract stamps and optionally find chamber and button features.

    Arguments:
        (str) image_path: path to the image file
        (tuple | namedtuple) corners: corner positions of the form ((ULx, ULy),(URx, URy),(LLx, LLy),(LRx, LRy))
        (object) device_dims: chip dimensions object with .x and .y attributes
        (bool) find_buttons: whether to find button features
        (bool) find_chambers: whether to find chamber features
        (float) stamp_width: width of the stamp to extract
        (ChipImage | None) reference_chip: reference chip to map features from
        (bool) save_summary_images: whether to save summary images
        (bool) coerce_chamber_center: whether to coerce chamber center to image center

    Returns:
        (np.ndarray) a 2-D array of Stamp objects with features found
    """
    # Calculate chamber center positions by interpolating corner positions
    centers = _quadrilateral_interpolation(corners, device_dims)

    # Create stamps from image
    stamps = _make_stamps(image_path, centers, device_dims, reference_chip.pinlist if reference_chip else None, stamp_width)
    ...
    for stamp in stamps.flatten():
        ...
        if reference_chip:
            ...
            # check if mutant is not to be processed
            if True:
                if find_chambers:
                    stamp.findChambers(coerce_center=coerce_chamber_center)
                if find_buttons:
                    # stamp.findButtons(coerce_center=coerce_button_center)
                    stamp.findButtons()

    # summarize all stamps
    summary = None
...
def process_images(
    ...
    if n_cpus > 1:
        ...
        for image_path in image_paths:
            # process_image(image_path, corners, device_dims, )
            pass
